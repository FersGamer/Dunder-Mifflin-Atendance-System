      ...
      <div class="flex-1 p-8 overflow-y-auto max-w-container-max mx-auto w-full">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div class="lg:col-span-8 flex flex-col gap-3">
            ...
            <!-- Filtros -->
            <div
              class="flex flex-wrap gap-2 mb-2 bg-surface-container-low p-2 rounded-lg border border-outline-variant w-fit">
              <button v-for="f in filtros" :key="f.key" @click="filtroActivo = f.key" :class="[
                ...
                {{ f.label }}
                <span v-if="f.key === 'solicitudes' && notifStore.pendientesCount > 0"
            ...
            <!-- Error -->
            <div v-else-if="notifStore.error"
              class="bg-error-container text-on-error-container rounded-lg p-4 text-body-sm border border-error/20">
              <span class="material-symbols-outlined align-middle mr-2 text-sm">error</span>
              {{ notifStore.error }}
            </div>

            <!-- Vacío -->
            <div v-else-if="notificacionesFiltradas.length === 0"
                ...
                <div class="flex flex-1 gap-4 p-5">
                  ...
                  <div class="flex-1 min-w-0">
                    <div class="flex justify-between items-center mb-2 gap-2 flex-wrap">
                      <span
                        class="font-label-caps text-[11px] tracking-wider bg-surface-container-high px-2.5 py-1 rounded text-on-surface border border-outline-variant">
                        {{ notif.etiqueta }}
                      </span>
                      <span class="font-mono text-label-caps text-on-surface-variant shrink-0">{{ notif.horaTexto
                      }}</span>
                    </div>
                    ...
                    <div v-if="notif.tipo === 'solicitud' && notif.aprobacion !== 'Pendiente'" class="mt-3">
                      <span class="font-label-caps text-[11px] tracking-wider px-3 py-1.5 rounded-full border"
                    ...
                    <div v-if="notif.tipo === 'solicitud' && notif.aprobacion === 'Pendiente'"
                      class="mt-4 flex flex-wrap gap-2">
                      <button @click="irAExpediente(notif.raw.empleado.id_empleado)"
                        class="font-label-caps text-[11px] tracking-wider bg-primary text-on-primary px-4 py-2 rounded-lg border border-primary hover:bg-primary-container hover:text-on-primary-container transition-colors flex items-center gap-1.5">
                        <span class="material-symbols-outlined text-body-md">person_search</span>
                        Revisar Expediente
                      </button>
                    </div>
                  </div>
          ...
          <div class="lg:col-span-4">
            <div class="bg-secondary-fixed border border-outline-variant rounded-lg p-6 shadow-sm sticky top-24">
              ...
              <div class="space-y-5">
                ...
                <div>
                  <label
                    class="block font-label-caps text-[11px] tracking-wider text-on-surface-variant mb-1.5">Departamento</label>
                ...
                <div class="grid grid-cols-2 gap-3">
                  <div>
                    <label class="block font-label-caps text-[11px] tracking-wider text-on-surface-variant mb-1.5">Fecha
                      Inicio</label>
                    <input v-model="form.fechaInicio" type="date"
                      class="w-full bg-surface border border-outline-variant rounded-lg text-body-sm focus:border-primary focus:outline-none h-10 px-2" />
                  </div>
                  <div>
                    <label class="block font-label-caps text-[11px] tracking-wider text-on-surface-variant mb-1.5">Fecha
                      Fin</label>
                ...
                <!-- Tipo -->
                <div>
                  <label class="block font-label-caps text-[11px] tracking-wider text-on-surface-variant mb-2">Tipo de
                    Reporte</label>
                  <div class="space-y-2.5">
                    <label v-for="t in tiposReporte" :key="t.value"
                      class="flex items-center gap-3 cursor-pointer group">
                      <input v-model="form.tipo" :value="t.value" type="radio"
                        class="text-primary focus:ring-primary" />
                      <span class="text-[14px] text-on-surface group-hover:text-primary transition-colors">{{ t.label
                      }}</span>
                    </label>
                  </div>
                </div>
                ...
                <div class="pt-2 border-t border-outline-variant">
                  <button @click="exportar" :disabled="reportesStore.generando"
                    ...
                    <span class="material-symbols-outlined text-sm"
                      :class="{ 'animate-spin': reportesStore.generando }">
                      {{ reportesStore.generando ? 'autorenew' : 'download' }}
                    </span>
                    {{ reportesStore.generando ? 'Generando...' : 'Exportar a CSV' }}
                  </button>

                  <p v-if="feedbackExport" class="mt-2 text-label-caps text-center font-medium"
                    :class="feedbackExport.ok ? 'text-status-punctual' : 'text-error'">
              ...
              <div class="mt-5 p-3 bg-surface-container-low border border-outline-variant rounded">
                <p class="font-mono text-[11px] text-on-surface-variant leading-relaxed">
                  HR-Notice: La falsificación de reportes de asistencia constituye una violación directa de las
                  políticas de la empresa y resultará en acciones disciplinarias severas.
                </p>
              </div>
            </div>
          </div>
...
const router = useRouter() // Enrutador para viajar al expediente
...
// Filtro inteligente: Solo muestra cosas de hoy o solicitudes que exijan acción
const notificacionesFiltradas = computed(() => {
  const hoy = new Date().toLocaleDateString('en-CA')
  const listaBase = notifStore.porFiltro(filtroActivo.value)

  return listaBase.filter(notif => {
    const esPendiente = notif.tipo === 'solicitud' && notif.aprobacion === 'Pendiente'

    const fechaOrigen = notif.raw.fecha || notif.raw.fecha_solicitud
    const fechaCorta = fechaOrigen ? fechaOrigen.split('T')[0] : ''
    const esDeHoy = fechaCorta === hoy
...
function irAExpediente(idEmpleado) {
  if (idEmpleado) {
    router.push(`/expediente/${idEmpleado}`)
  }
}
